...
lsp.on_attach(function(client, bufnr)
  ...
  vim.keymap.set("n", "<leader>a", function() vim.lsp.buf.code_action() end, { desc = '[A]ctions for code', buffer = bufnr, remap = false })
  vim.keymap.set("n", "<leader>f", function() vim.lsp.buf.format() end,      { desc = '[Format] code', buffer = bufnr, remap = false })
end)
